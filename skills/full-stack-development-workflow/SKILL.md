...
1. 定义用户可观察的成功结果和当前基线。
2. 从第一性原理检查问题定义：拆出不可消除的目标、约束与基础机制，区分必需成本和沿用假设，再推导方案；不要直接接受现成解法或只修表面症状。
3. 找到完整调用链、稳定接口、真实约束和最小改动位置。
4. 需要拆分时按端到端行为建立纵向 Issue；一次只推进一个可验收切片。
5. 优先复用现有代码、标准库、平台能力和已安装依赖，再写最少新代码。
6. 非平凡逻辑留下能捕获真实失败的最小检查；Bug 先建立可变红的反馈环。
7. 功能完成后切换到对抗式审查：主动用非法输入、边界状态、并发、重试、断网和部分失败攻击实现，修复发现并重跑回归；未经明确授权不为此启动多 Agent。
