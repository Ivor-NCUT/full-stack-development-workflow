...
需要远端协作、持续跟踪或仓库明确规定时，用一个内聚 Issue／分支／PR 管理变更；本地小改动不强制创建这些材料。会触碰同一文件的多个 Agent 串行执行或使用隔离 worktree，不共享未提交写入。
...
切片按端到端行为组织。只有确定需要并行或跨会话时才拆多个 Issue；简单修改需要 Issue 时保留一个内聚任务；没有跟踪需求不为流程新增 Issue。
...
需要 PR 时用 `gh pr create`。用户只要 PR 就在 PR 验收后交付；已明确授权合并则必要检查通过后继续合并并回读，不重复问是否可合并。未授权合并时先准备具体 PR，再只询问该动作。CI 与分支保护继续遵守。
