...
主入口只保留任务判断和统一执行循环，细节按需加载四份一层参考：
...
参考文件提供流程和安全边界，不复制连接器或外部插件实现。Ponytail 保持为可选的外部 Codex 插件：父 Skill 在编码任务中按需使用它，未安装时执行同一能力阶梯；仓库不静默安装插件或信任生命周期 Hook。GitHub 远端操作使用 `gh` / `gh api`；Zeabur 操作调用已安装的 `zeabur-*` Skills。
